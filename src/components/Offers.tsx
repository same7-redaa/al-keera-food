...
export const Offers: React.FC = () => {
  ...
  return (
    <section id="offers" className="py-12 sm:py-16 lg:py-20 lg:min-h-[80vh] flex items-center relative bg-brand-primary/40 overflow-hidden border-y border-brand-gold/20 scroll-mt-24">
      
      {/* Background Lighting */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-brand-gold/10 rounded-full blur-[130px] pointer-events-none"></div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-5 relative reveal-scale">
            <div className="relative h-64 sm:h-80 lg:h-96 rounded-3xl overflow-hidden border-2 border-brand-gold/40 shadow-2xl group">
              <img
              ...
              <div className="absolute bottom-3 inset-x-3 bg-brand-deep/90 backdrop-blur-md p-2.5 rounded-xl border border-brand-gold/30 text-center">
                <span className="text-xs font-bold text-brand-brightGold">يكفي 3 إلى 4 أفراد براحة!</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 flex flex-col justify-between reveal delay-100">
            <div>
              <div className="flex items-center gap-2 text-xs font-bold text-brand-brightGold mb-2">
                <Clock className="w-4 h-4" />
                <span>عرض اليوم لفترة محدودة</span>
              </div>

              <h3 className="text-2xl sm:text-3xl lg:text-4xl font-black text-brand-cream mb-2">
                {MAIN_OFFER.title}
              </h3>

              <p className="text-xs sm:text-sm text-brand-cream/75 mb-5 leading-relaxed">
                {MAIN_OFFER.description}
              </p>

              {/* Items Included List */}
              <div className="bg-brand-deep/80 rounded-2xl p-4 border border-brand-gold/20 mb-6">
                <span className="text-xs font-bold text-brand-brightGold block mb-2.5">
                  محتويات العرض الملكي:
                </span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {MAIN_OFFER.itemsIncluded.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-xs text-brand-cream/90 font-medium">
                      <Check className="w-3.5 h-3.5 text-brand-brightGold flex-shrink-0" />
                      <span>{item}</span>
                    </div>
                  ))}
                </div>
            ...
            {/* Price & Action */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-brand-gold/20">
              <div className="flex items-baseline gap-2">
                <span className="text-3xl sm:text-4xl font-black text-brand-brightGold">
                  {MAIN_OFFER.discountedPrice}
                </span>
                <span className="text-xs text-brand-cream/80 font-medium">جنيه فقط</span>
                <span className="text-base text-brand-cream/40 line-through font-bold mr-2">
